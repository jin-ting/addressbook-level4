...
----
The UI outcome of scrolling and selecting a PersonCard in the `PersonListPanel` is shown below:
...
//tag::FindCommand[]
=== FindCommand by any searched field


The find function will support the search of contact by any keywords. The keywords must be a full word match. The command will inherit from `FindCommand` .The find function only deals with existing parameters `Name`, `Email`, `Address`, `Phone`and `Tags`. Such parameters inherit from `Person` which resides inside `Model`.
`Find Command` will only execute when the prefix of the parameters is given. Any other prefix as input will be considered as invalid input.
...
A successful tokenism of lead to the keywords to be allocated to its `containsKeyWordPredicate` under `Model` based on the input prefix.Addressbook will search for the keywords in the revelvant parameters.
Upon any successful match, the particulars of the contact will be displayed. The sketch belows indicates the various outcome of search function by  using` FindCommand` .

image::FindCommand_Name.PNG[width="200"]
image::FindCommand_Phone.PNG[width="200"]
image::FindCommand_Email.PNG[width="200"]
image::FindCommand_Address.PNG[width="200"]
image::FindCommand_Tag.PNG[width="200"]
